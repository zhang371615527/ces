...
namespace AnyWise.HWPMS.DALSQL
{
    ...
    /// 绩效分配参数表
    /// </summary>
    public class Yljxfpcsb : BaseDALSQL<YljxfpcsbInfo>, IYljxfpcsb
    {
        #region 对象实例及构造函数

        public static Yljxfpcsb Instance
        {
            get
            {
                return new Yljxfpcsb();
            }
        }
        public Yljxfpcsb()
            : base("TB_Yljxfpcsb", "ID")
        {
        }
        ...
        protected override YljxfpcsbInfo DataReaderToEntity(IDataReader dataReader)
        {
            YljxfpcsbInfo info = new YljxfpcsbInfo();
        ...
        protected override Hashtable GetHashByEntity(YljxfpcsbInfo obj)
        {
            YljxfpcsbInfo info = obj as YljxfpcsbInfo;
        ...
        public bool InsertBatch(List<YljxfpcsbInfo> list)
        {
            ...
            if (sqlDbTransaction != null)
            {
                ...
                {
                    //int seq = 1;
                    foreach (YljxfpcsbInfo detail in list)
                    {
