...
namespace AnyWise.HWPMS.DALSQL
{
    ...
    /// 月度目标子表
    /// </summary>
	public class Ydmbzb : BaseDALSQL<YdmbzbInfo>, IYdmbzb
	{
		#region 对象实例及构造函数

		public static Ydmbzb Instance
		{
			get
			{
				return new Ydmbzb();
		...
		public Ydmbzb() : base("BIZ_YDMBZB","ID")
		{
		}
		...
		protected override YdmbzbInfo DataReaderToEntity(IDataReader dataReader)
		{
			YdmbzbInfo info = new YdmbzbInfo();
			SmartDataReader reader = new SmartDataReader(dataReader);
			
			info.ID = reader.GetString("ID");
			info.YdmbId = reader.GetString("YDMB_ID");
			info.Type = reader.GetString("Type");
			info.Level = reader.GetString("Level");
			info.KhzbId = reader.GetString("KHZB_ID");
			info.MxkhzbId = reader.GetString("MXKHZB_ID");
        ...
        protected override Hashtable GetHashByEntity(YdmbzbInfo obj)
		{
		    YdmbzbInfo info = obj as YdmbzbInfo;
			Hashtable hash = new Hashtable(); 
			
			hash.Add("ID", info.ID);
 			hash.Add("YDMB_ID", info.YdmbId);
 			hash.Add("Type", info.Type);
 			hash.Add("Level", info.Level);
 			hash.Add("KHZB_ID", info.KhzbId);
 			hash.Add("MXKHZB_ID", info.MxkhzbId);
        ...
        public override Dictionary<string, string> GetColumnNameAlias()
        {
            ...
            //dict.Add("ID", "编号");
            dict.Add("ID", "内码");
             dict.Add("YdmbId", "月度目标内码");
             dict.Add("Type", "目标类型");
             dict.Add("Level", "目标等级");
             dict.Add("KhzbId", "目标内容");
             dict.Add("MxkhzbId", "考核指标（二级）");
             dict.Add("Unit", "单位");
